...
*/
	 ...
	 */
	public static function get_feature_instance( $class ) {

		global $plethora;
		if ( !empty( $plethora['instances'][$class] ) && is_object( $plethora['instances'][$class] ) ) {

			return $plethora['instances'][$class];
		}
	...
	public static function container_part_output() {
		...
		foreach ( $container_parts as $key => $part ) {
			...
			if ( empty( $part['loaded'] ) || ( !is_singular() && !is_404() && $this_filter === 'plethora_content_main_loop' ) ) { 
				...
				// Display developer notes opening
				self::dev_comment( $part['dev_notes_before'], 'customization_info' );
				...
				// Display developer notes opening
				self::dev_comment( $part['dev_notes_after'], 'customization_info' );
