    ...
    */
    public function slide_options() {
      ...
      $slide_options['caption-transparentfilm-xenia'] = array(
          ...
          'type'    => 'button_set',
          'title'   => esc_html__( 'Caption Background Transparency', 'plethora-framework' ),
          'options' => array( '' => 'No', 'transparent_film' => esc_html__( 'Semi Transparent' ), 'transparent' => 'Transparent', ),
      );
        ...
        foreach ($slider['slide_image'] as $key => $slide ) {
         if ( !empty($slide) ) { 
            ...
            if ( $status && isset( $fullimg[0] ) && !empty( $fullimg[0] ) ) { 
              ...
              $slide['colorset']                 = esc_attr( $slider['slide_colorset'][$key] );
              $slide['transparentfilm']          = esc_attr( $slider['slide_transparentfilm'][$key] );
              ...
              $slide['caption_colorset']         = esc_attr( $slider['slide_caption_colorset'][$key] );
              $slide['caption_transparentfilm']  = esc_attr( $slider['slide_caption_transparentfilm'][$key] );
              $slide['caption_size']             = esc_attr( $slider['slide_caption_size'][$key] );
              $slide['caption_align']            = esc_attr( $slider['slide_caption_align'][$key] );
              $slide['caption_textalign']        = esc_attr( $slider['slide_caption_textalign'][$key] );
              $slide['caption_neutralizetext']   = esc_attr( $slider['slide_caption_neutralizetext'][$key] );
              $slide['caption_animation']        = esc_attr( $slider['slide_caption_animation'][$key] );
              $slide['caption_headingstyle']     = esc_attr( $slider['slide_caption_headingstyle'][$key] );
              ...
              $slide['caption_buttonstyle']      = esc_attr( $slider['slide_caption_buttonstyle'][$key] );
