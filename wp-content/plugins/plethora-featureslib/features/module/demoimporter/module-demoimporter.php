...
//// AJAX ONLY METHODS START
    ...
    public function import_terms() { 
      ...
      foreach ( $this->terms as $term ) {
        ...
        $id = wp_insert_term( $term->name, $term->taxonomy, $termarr );
        if ( ! is_wp_error( $id ) ) {

          if ( isset( $term->term_id ) ){ // if we have a term_id, then move on with term meta

            if ( isset( $term->termmeta ) ) { // make sure it is compatible with older importer versions 
              foreach ( $term->termmeta as $meta_key => $meta_vals ) {

                foreach ( $meta_vals as $meta_val ) {

                  add_term_meta( $term->term_id, $meta_key, $meta_val );
                }
              }
            }
          ...
          $this->errors['terms']['dev'][] = $id->get_error_message();
          continue;
        }
      }
