	...
	class Plethora_Module_Woocommerce {
		...
		public function __construct() {
			...
			# Admin: theme Options & metaboxes
				...
				// Minicart ( append in 'Header' section and set status variable )
				add_filter( 'plethora_themeoptions_header', array($this, 'minicart_themeoptions'), 120);	// Theme Options // Account+Cart icons menu
				$this->minicart = Plethora_Theme::option( THEMEOPTION_PREFIX .'woo-minicart', 1 );
			...
			# Front: Templates Setup ( hook on 'get_header' please )
	      		add_action( 'get_header', array( $this, 'template_parts') );    // Layout containers setup 
			...
			# Front: Other configuration
				...
				// Ajax refresh for Plethora mini cart
				if ( $this->minicart ) {

		    		add_filter( 'woocommerce_add_to_cart_fragments', array( $this, 'template_part_minicart' ) ); 
				}
			}
		}
		...
		/**
		 * Setup auxiliary Plethora template parts
		 */
	    public function template_parts() {
	    	...
	    	if ( $this->minicart ) {
	    		...
	    		add_action( 'plethora_navigation_after', array( $this, 'template_part_minicart' ) ); 
	    	}

	    }

		/**
		 * Load Mini Cart template part
		 * Usually hooked in 'plethora_navigation_after' action
		 * Used also for Ajax reload when hooked on 
		 * 'woocommerce_add_to_cart_fragments' filter
		 */
	    public function template_part_minicart( $fragments = array() ) {
	    	...
	    	$options = array(
				...
				'cart_icon'          => Plethora_Theme::option( THEMEOPTION_PREFIX .'woo-minicart-cart-icon',  'lin-ecommerce icon-ecommerce-cart', 0, 0 ),
				...
				'account'            => Plethora_Theme::option( THEMEOPTION_PREFIX .'woo-minicart-account', 1, 0, 0 ),
				'account_icon'       => Plethora_Theme::option( THEMEOPTION_PREFIX .'woo-minicart-account-icon',  'lin icon-basic-gear', 0, 0 ),
	    	...
	    	if ( current_filter() === 'woocommerce_add_to_cart_fragments' ) {
		    	...
		    	get_template_part( 'templates/header/woo_minicart' );
		    	$fragments['#ple_woocart'] = ob_get_clean();
		    	return $fragments;
	    	}
	    	// not an ajax call, just return the template part
	    	set_query_var( 'options', $options );
	    	get_template_part( 'templates/header/woo_minicart' );
	    }
		...
		*/
		...
		/**
		* Special options for mini cart menu
		* Hooked on 'plethora_themeoptions_header'
		*/
		public function minicart_themeoptions( $sections ) {

        	// setup theme options according to configuration
			$opts        = $this->minicart_options();
			$opts_config = $this->minicart_options_config();
			$fields      = array();
        	foreach ( $opts_config as $opt_config ) {
		    ...
		    $sections[] = array(
				'title'      => esc_html__('Mini Cart ( Woo )', 'plethora-framework'),
				'heading'    => esc_html__('MINI CART ( WooCommerce )', 'plethora-framework'),
				'subsection' => true,
				'fields'     => $fields
				);
			}
		...
		*/
		...
		/** 
		* Returns MINI CART options index
		* It contains ALL possible mini cart options, no matter which theme
		*/
        public function minicart_options() {

			$minicart_options['minicart'] = array(
							'id'      => THEMEOPTION_PREFIX .'woo-minicart',
							'type'    => 'switch', 
							'title'   => esc_html__('Display Mini Cart', 'plethora-framework'),
							'desc'    => esc_html__('Will display mini cart icon next to navigation', 'plethora-framework'),
			);

			$minicart_options['cart-section-start'] = array(
							'id'       => 'woo-minicart-cart-start',
							'type'     => 'section',
							'title'    => esc_html__('Cart Icon Options', 'plethora-framework'),
							'indent'   => true,
							'required' => array( THEMEOPTION_PREFIX .'woo-minicart','=', true ),
			);

			$minicart_options['cart-icon'] = array(
			...
			$minicart_options['cart-icon-size']	= array(
			...
			$minicart_options['cart-title']	= array(
			...
			$minicart_options['cart-count']	= array(
			...
			$minicart_options['cart-count-color']	= array(
							...
							'required'    => array( 
												array( THEMEOPTION_PREFIX .'woo-minicart','=', true ),
												array( THEMEOPTION_PREFIX .'woo-minicart-cart-count','=', true ),
										  	 ),
			);

			$minicart_options['cart-count-bgcolor']	= array(
			...
			$minicart_options['cart-total']	= array(
			...
			$minicart_options['cart-section-end'] = array(
			...
			$minicart_options['account-section-start'] = array(
							'id'       => 'woo-minicart-account-start',
							'type'     => 'section',
							'title'    => esc_html__('Account Icon Options', 'plethora-framework'),
							'indent'   => true,
							'required' => array( THEMEOPTION_PREFIX .'woo-minicart','=', true ),
			);

			$minicart_options['account'] = array(
							'id'       => THEMEOPTION_PREFIX .'woo-minicart-account',
							'type'     => 'switch', 
							'title'    => esc_html__('Display My Account Link Icon', 'plethora-framework'),
							'desc'     => esc_html__('Will display My Account icon next to navigation', 'plethora-framework'),
							'required' => array( THEMEOPTION_PREFIX .'woo-minicart','=', true ),
			);

			$minicart_options['account-icon'] = array(
			...
			$minicart_options['account-icon-size'] = array(
			...
			$minicart_options['account-title-visit'] = array(
							...
							'required'  => array( 
												array( THEMEOPTION_PREFIX .'woo-minicart','=', true ),
												array( THEMEOPTION_PREFIX .'woo-minicart-account','=', true )
										  ),
			);
			$minicart_options['account-title-login'] = array(
			...
			$minicart_options['account-section-end'] = array(
		...
		*/
		...
		/** 
		* Mini cart options config for theme options
		*/
		public function minicart_options_config() {

			$minicart_options_config = array(
						array( 
							'id'                    => 'minicart', 
							'theme_options'         => true, 
							'theme_options_default' => true,
						...
						array( 
							'id'                    => 'cart-icon', 
							'theme_options'         => true, 
							'theme_options_default' => 'lin-ecommerce icon-ecommerce-cart',
							'metabox'               => false,
							'metabox_default'       => NULL
							),
						array( 
						...
						array( 
							'id'                    => 'account-icon', 
							'theme_options'         => true, 
							'theme_options_default' => 'lin icon-basic-gear',
							'metabox'               => false,
							'metabox_default'       => NULL
							),
						array( 
			...
			return $minicart_options_config;
		}
