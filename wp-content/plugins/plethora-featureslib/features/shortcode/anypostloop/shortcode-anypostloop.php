	...
	class Plethora_Shortcode_Anypostloop extends Plethora_Shortcode { 
		...
		public function __construct( $post_type_obj, $type = 'grid' ) {
			...
			if ( $this->sc_type === 'slider' ) {
				...
				$this->sc_name   = sprintf( esc_html__('%1$s Slider', 'plethora-framework'), $this->post_type->label );
				...
				$this->sc_name   = sprintf( esc_html__('%1$s Grid', 'plethora-framework'), $this->post_type->label );
		...
		*/
		public function params_config() {
			...
			$params_config[] = array( 'id' => 'autoplay', 						'default' => '1', 						'field_size' => '', );
	   ...
	   */
	   public function params_index() {
			...
			$params['items_template'] = array(
				...
				'type'        => 'dropdown',
				'heading'     => sprintf( esc_html__('%s Template', 'plethora-framework'), ucfirst( $this->sc_type ) ),
			...
			$params['items_link_behavior'] = array(
				...
				'description'      => esc_html__('Select secondary taxonomy to be displayed', 'plethora-framework'),
				'value'            => array( 
					sprintf( esc_html__('Link to %1$s page', 'plethora-framework'), strtolower( $this->post_type->labels->singular_name ) )                 => 'normal',
					sprintf( esc_html__('Link to %1$s page, in new tab', 'plethora-framework'), strtolower( $this->post_type->labels->singular_name ) )     => 'blank',
					sprintf( esc_html__('Link to %1$s page, in ajax window', 'plethora-framework'), strtolower( $this->post_type->labels->singular_name ) ) => 'ajax',
					sprintf( esc_html__('Do not link', 'plethora-framework'), strtolower( $this->post_type->labels->singular_name ) )                       => 'no_link',
				),
				'dependency'       => array( 
					'element' => 'items_template', 
					'value'   => $this->get_templates_dependency( array( 
		...
		*/
		public function get_templates_dependency( $args = array() ) {
			...
			return !empty( $templates_dependency ) ? array_unique( $templates_dependency ) : array( 'foo' );
		}
