...
class Plethora_Shortcode_Bookingcallto extends Plethora_Shortcode { 
    ...
    */
    ...
    #FIELDS CONFIG TAB
        ...
        $params_index['adults_name'] = array( 
              ...
              'description' => sprintf( esc_html__('Must match the %1$adults field name%2$s attribute on the targeted %1$sCF7 booking form%2$s.%3$sCF7 form field sample configuration: %1$s[select adults default:get "1" "2" "3" "4"]%2$s', 'plethora-framework'), '<strong>', '</strong>', '<br>' ),
              'dependency'  => array( 
                                  'element' => 'adults', 
                                  'value'   => array( 'true' ),  
                                        )
        ...
        $params_index['children_name'] = array( 
              ...
              'description' => sprintf( esc_html__('Must match the %1$children field name%2$s attribute on the targeted %1$sCF7 booking form%2$s.%3$sCF7 form field sample configuration: %1$s[select children default:get "1" "2" "3" "4"]%2$s', 'plethora-framework'), '<strong>', '</strong>', '<br>' ),
              'dependency'  => array( 
                                  'element' => 'children', 
                                  'value'   => array( 'true' ),  
                                        )
    ...
    */
    public function content( $atts, $content = null ) {
        ...
        if ( $date_arrival || $date_departure ) {
            ...
            $script_args = array(
                ...
                'script' => self::get_datepicker_script(),
                'multiple' => false,
            );
            Plethora_Theme::enqueue_init_script( $script_args );           
        }     
    ...
    public static function get_field_config( $name, $title = '', $val = '', $colsize = '', $id = '', $placeholder = '', $multiple = false  ) {

        return array(
            'field_is_select'   => is_array( $val ) ? true : false,
            'field_is_input'    => is_array( $val ) ? false : true,
            'field_colsize'     => esc_attr( $colsize ),
            'field_label'       => esc_html( $title ),
    ...
    public static function get_datepicker_script() {
